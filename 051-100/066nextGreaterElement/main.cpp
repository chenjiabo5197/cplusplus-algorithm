...
public:
    vector<int> nextGreaterElement(vector<int>& nums1, vector<int>& nums2) {
        // 储存key为nums中元素，value为比当前元素大的数
        unordered_map<int, int> numBig;
        // 对于nums2，最右侧数字没有比它更大的值，所以为-1
        numBig[nums2[nums2.size()-1]] = -1;
        for (int i = nums2.size()-2; i >=0 ; --i) {
            if(nums2[i] < nums2[i+1])
            {
                numBig[nums2[i]] = nums2[i+1];
            }
            else if (nums2[i] < numBig[nums2[i+1]])
            {
                numBig[nums2[i]] = numBig[nums2[i+1]];
            }
            else
            {
                int index = numBig[nums2[i+1]];
                while (index != -1)
                {
                    if (nums2[i] < numBig[index])
                    {
                        numBig[nums2[i]] = numBig[index];
                        break;
                    }
                    index = numBig[index];
                }
